...
! variables
  ...
  namelist /comppar/ lambda, mobility, betav, deltat, numtimesteps, &
...
!   define gradient projection problem
  ...
  ! open(unit=11, file='snapshots.txt', status='replace')
  ! open(unit=14, file='pressure.txt', status='replace')
  ! open(unit=15, file='mesh_coor2.txt', status='replace')
  ! open(unit=40, file='c-trace.txt', status='replace')
...
!   write trace of conformation tensor as a separate scalar VTK
    call create_vector ( problemc, ctrace, vec=2 )
    call create_subscript ( mesh, problemc, trace_sub, degfd=1, vec=2 )
    ctrace%u(trace_sub%s) = ctensor%u(cxx%s) + ctensor%u(cyy%s)
    write(filename,'(a,i4.4,a)') 'trace', ipost, '.vtk'
    call write_scalar_vtk ( mesh, problemc, vector=ctrace, &
      dataname='trace_c', filename=filename )
    call delete ( trace_sub )
    call delete ( ctrace )
