"""
Generate TEST data for the 2-parameter (λ, βv) extrudate swell experiment.

Test design: 2D Latin Hypercube Sampling (steady-state only).
  λ  ∈ [1.5, 14.5]
  βv ∈ [0.12, 0.85]
...
def main():
    ...
    betav_min,  betav_max  = 0.06, 0.8

    sampler = qmc.LatinHypercube(d=2, seed=seed)
    lhs_raw = sampler.random(n_samples)               
    scaled = qmc.scale(
        lhs_raw,
    ...
    for i in range(n_samples):
        print(f"  [{i:3d}] λ={lambda_values[i]:.4f}  β={betav_values[i]:.4f}")
...
def move_vtk_files(lam, betav, vtk_dir):
    folder = os.path.join(vtk_dir, f"lambda-{lam:.3f}_beta-{betav:.3f}")
...
def run_simulations(executable, lambda_vals, betav_vals, alphapar_fixed,
    ...
    for i in range(n_samples):
        ...
        print(f"\n[{i+1}/{n_samples}] Running λ={lam:.3f}, β={betav:.3f}, α={alphapar_fixed:.3f} (fixed)")
        ...
        move_vtk_files(lam, betav, vtk_dir)
